...
#ifndef RANDO_PCG_RANDOM_H
#define RANDO_PCG_RANDOM_H
...
RANDO_DEFINE_CONSTANT(uint8_t, default, multiplier, 141U)
RANDO_DEFINE_CONSTANT(uint8_t, default, increment, 77U)

RANDO_DEFINE_CONSTANT(uint16_t, default, multiplier, 12829U)
RANDO_DEFINE_CONSTANT(uint16_t, default, increment, 47989U)

RANDO_DEFINE_CONSTANT(uint32_t, default, multiplier, 747796405U)
RANDO_DEFINE_CONSTANT(uint32_t, default, increment, 2891336453U)

RANDO_DEFINE_CONSTANT(uint64_t, default, multiplier, 6364136223846793005ULL)
...
protected:
    static itype
    advance(itype state, itype delta, itype cur_mult, itype cur_plus);

    static itype
    distance(itype cur_state,
             itype newstate,
             itype cur_mult,
             itype cur_plus,
             itype mask = ~itype(0U));
    ...
    itype
    distance(itype newstate, itype mask = itype(~itype(0U))) const
    {
...
engine<xtype,
       ...
       output_previous,
       stream_mixin,
       multiplier_mixin>::advance(itype state,
                                  itype delta,
                                  itype cur_mult,
                                  itype cur_plus)
{
...
engine<xtype,
       ...
       output_previous,
       stream_mixin,
       multiplier_mixin>::distance(itype cur_state,
                                   itype newstate,
                                   itype cur_mult,
                                   itype cur_plus,
                                   itype mask)
...
using oneseq_base              = engine<xtype,
                           itype,
                           output_mixin<xtype, itype>,
                           output_previous,
                           oneseq_stream<itype>>;
...
template <typename xtype,
          ...
          template <typename XT, typename IT> class output_mixin,
          bool output_previous = (sizeof(itype) <= 8)>
using unique_base              = engine<xtype,
                           itype,
                           output_mixin<xtype, itype>,
                           output_previous,
                           unique_stream<itype>>;
...
template <typename xtype,
          ...
          template <typename XT, typename IT> class output_mixin,
          bool output_previous = (sizeof(itype) <= 8)>
using setseq_base              = engine<xtype,
                           itype,
                           output_mixin<xtype, itype>,
                           output_previous,
...
RANDO_DEFINE_CONSTANT(uint8_t, mcg, multiplier, 217U)
RANDO_DEFINE_CONSTANT(uint8_t, mcg, unmultiplier, 105U)

RANDO_DEFINE_CONSTANT(uint16_t, mcg, multiplier, 62169U)
RANDO_DEFINE_CONSTANT(uint16_t, mcg, unmultiplier, 28009U)

RANDO_DEFINE_CONSTANT(uint32_t, mcg, multiplier, 277803737U)
RANDO_DEFINE_CONSTANT(uint32_t, mcg, unmultiplier, 2897767785U)

RANDO_DEFINE_CONSTANT(uint64_t, mcg, multiplier, 12605985483714917081ULL)
...
struct inside_out : private baseclass
{
    ...
    static bool
    external_step(result_type& randval, size_t i)
    {
    ...
    static bool
    external_advance(result_type& randval,
                     size_t i,
                     result_type delta,
                     bool forwards = true)
    {
...
template <bitcount_t table_pow2,
...
private:
    ...
    static constexpr state_type tick_mask = may_tick
    ...
    RANDO_NOINLINE void
    advance_table(state_type delta, bool isForwards = true);
...
public:
    ...
    void
    advance(state_type distance, bool forwards = true);

    void
    backstep(state_type distance)
    {
...
private:
    ...
    selfinit();
    void
    datainit(const result_type* data);
...
public:
    ...
    template <bitcount_t table_pow2_,
              bitcount_t advance_pow2_,
              typename baseclass_,
              typename extvalclass_,
              bool kdd_>
    friend bool
    operator==(const extended<table_pow2_,
                              advance_pow2_,
                              baseclass_,
                              extvalclass_,
                              kdd_>&,
               const extended<table_pow2_,
                              advance_pow2_,
                              baseclass_,
                              extvalclass_,
                              kdd_>&);
...
template <bitcount_t table_pow2,
          ...
          bool kdd>
void
extended<table_pow2, advance_pow2, baseclass, extvalclass, kdd>::datainit(
    ...
    state_type delta, bool isForwards)
{
    ...
    for (size_t i = 0; i < table_size; ++i)
    {
        ...
        carry += insideout::external_advance(
            data_[i], i + 1, trunc_delta, isForwards);
    }
}
...
template <bitcount_t table_pow2,
          ...
          bool kdd>
void
extended<table_pow2, advance_pow2, baseclass, extvalclass, kdd>::advance(
