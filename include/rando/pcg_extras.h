...
shuffle(Iter from, Iter to, RandType&& rng)
{
    ...
    typedef
        typename std::remove_reference<RandType>::type::result_type result_t;
    auto count = to - from;
    while (count > 1)
    {
...
public:
    ...
    explicit seed_seq_from(Args&&... args) : rng_(std::forward<Args>(args)...)
    {
        // Nothing (else) to do...
    }
...
{
private:
    static constexpr IntType
    fnv(IntType hash, const char* pos)
    {
        return *pos == '\0' ? hash : fnv((hash * IntType(16777619U)) ^ *pos,
                                         (pos + 1));
    }
